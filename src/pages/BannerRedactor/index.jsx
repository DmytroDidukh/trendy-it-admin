...
const BannerRedactor = ({id, editMode}) => {
    ...
    const {banner, sliderImage, imagesToDelete} = useSelector(({Banners, Images}) => ({
    ...
    const onSaveBanner = () => {
        if (bannerObj.title && bannerObj.description) {
            ...
            if (bannerObj.toSlider && !sliderImage) {
                window.alert('Додайте зображення, якщо бажаєте відобразити баннер на сайті')
                return
            }
    ...
    const onGoBack = () => {
        ...
        if (editMode) {
            const {image} = banner
            const notSavedSliderImage = image && image.publicId === sliderImage.publicId ? null : sliderImage;
