...
const ImagePlaceholder = () => {
  ...
  const onSliderScroll = ({ target: { id } }) => {
    ...
    const currentPosition =
      (transformValue && transformValue.match(/-*\d+/)[0]) || 0;

    if (id === 'slider-left' && sliderPosition > 0) {
      setSliderPosition(sliderPosition - 1);
      sliderImages.current.style.transform = `translateX(${
        +currentPosition + 300
      }px)`;
  ...
  const onDeleteImage = () => {
    if (
      sliderPosition !== images.length &&
      window.confirm('Видалити зображення?')
    ) {
      dispatch(
        setImagesToDeleteAndUpdateState(images[sliderPosition].publicId)
      );
    }
  };
    ...
    <div className='image-placeholder'>
      <div className='image-placeholder__container'>
        <div className='image-placeholder__images' ref={sliderImages}>
          {images.map((img) => (
            <div
              key={img.publicId || Math.random()}
              className='image-placeholder__images__item'
